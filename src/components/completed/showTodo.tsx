...
export default function ShowTodo({
  ...
  const [taskUpdatingData, setTaskUpdatingData] = useState({
    id: "",
    completed: true,
    task: true,
  });
    ...
    try {
      ...
      if (target) {
        ...
        const res = await fetch(`http://localhost:3000/api/task/${taskId}`, {
          ...
          body: JSON.stringify({ important: updateImportant, _id: taskId }),
        });

        if (res.ok) {
          setTaskUpdatingData((prev: any) => ({
            ...prev,
            id: taskId,
        ...
        (item: any, index: number): ReactElement => (
          <div key={index}>
            {!item.completed || !item ? (
              ...
              <div className="rounded-md  m-2 flex justify-between tasks text-[1.2em]  bg-[#2a2828] text-[#d1c8c8] z-[-999] cursor-pointer ">
                <div className="flex items-center">
                  <div
                    className="mr-2  circle ml-2 "
                    data-completeTask-id={item._id}
                    onClick={handleCompleteClick}
                  >
                    {item.completed ? (
                      <FaRegCircleCheck className="text-[#2db42d]" />
                    ) : (
                      <FaRegCircle />
                    )}
                  </div>
                ...
                <div className="cursor-pointer ">
                  {item.important ? (
                    <FaRegStar className="text-[#484846] mt-4 mr-2" />
                  ) : (
                    <FaStar className="mt-4 mr-2" />
                  )}{" "}
                </div>
              </div>
            )}
