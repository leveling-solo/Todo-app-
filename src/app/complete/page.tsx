...
import ShowTodo from "@/components/completed/showTodo";

import { BsLayoutTextWindowReverse } from "react-icons/bs";

export default function completePage(): ReactElement<Element> {
  ...
  async function getingData() {
    try {
      ...
      let filterData = d.data.filter((item: any): any => item.completed);
      setInfo(filterData);
  ...
  useEffect(() => {
    const savedBgColor: any = localStorage.getItem("completeBgcolor");
    if (savedBgColor) {
  ...
  function handleColorClick(event: any) {
    ...
    localStorage.setItem("completeBgcolor", selectedColor);
  }
      ...
      <div className="flex  justify-between items-center m-2 relative ">
        <div className="flex  items-center">
          <p className="date font text-[1.5rem] font-[500]">
            <FaRegCheckCircle className="mr-3 text-[lightgreen]" />
          </p>
          <h1 className=" date text-[2rem]">Completed</h1>
        </div>
